...
struct FamilyShareReconcilerTests {
    private func makeContext(cloudKit: MockCloudKitService = MockCloudKitService()) -> ( // swiftlint:disable:this large_tuple
        ...
        return (reconciler, cloudKit, appState, zoneID, family, hero)
    }

    // MARK: - Absence Convergence

    @Test
    func `single participant-list absence does not deactivate a profile`() async throws {
        let ctx = makeContext()
        ...
        let fresh = try await ctx.cloudKit.fetch(Profile.self, id: ctx.hero.id)
        #expect(fresh.isActive)
    }

    @Test
    func `two consecutive absences deactivate the profile`() async throws {
        let ctx = makeContext()
        await ctx.reconciler.reconcileIfOwner()
        await ctx.reconciler.reconcileIfOwner()
    ...
    func `present identity keeps the profile active and clears absence marks`() async throws {
        ...
        await ctx.reconciler.reconcileIfOwner()
        #expect(try await ctx.cloudKit.fetch(Profile.self, id: ctx.hero.id).isActive)
    ...
    @Test
    func `identity resolution failure aborts the pass without deactivating`() async throws {
