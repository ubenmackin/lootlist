...
- Strict constraints:
  ...
  - All mutations go through Services → `CacheService` write + `CKSyncEngineCoordinator.enqueueSave/enqueueDelete` (except the DEBUG-only schema seed, §2 — ephemeral `schemaseed-` rows that never touch `CacheService`/`enqueue`/`ingest`).
  - Engine-delivered and query-derived server→cache record writes ride ONE entry point: `CKSyncEngineDelegateHandler.ingest(records:databaseScope:zoneID:notifiesOnCompletion:)`. Exactly three sanctioned exceptions: non-owner cache reconciliation via `BackgroundCacheActor.reconcileParticipantSet` (§4) and pre-session bootstrap mirrors in FamilyService during family creation/join, where ingest fails closed before a session exists (the owner-side §4 snapshot pass is NOT an exception — it rides ingest() via `handleIncomingRecordsDirectly`), plus the DEBUG-only schema seed via `SchemaSeedService.pushSchemaSeed` (invoked from `iCloudStatusView`, `#if DEBUG` only) — direct `cloudKit.save`/`delete` bypassing `CacheService`, `enqueueSave`/`enqueueDelete`, and `ingest()`; scope is reserved `schemaseed-` record names in the active family zone only, never touches `CacheService`/`ingest`/`enqueue`, best-effort delete on all paths, `Family` never seeded (existing family is the parent reference only). Never query-then-upsert directly into cache outside those three doors.
  ...
  - Services and AppState depend on `any CloudKitServiceProtocol` (narrower seams exist where useful). Do not concrete-type CloudKit dependencies.
   - UserDefaults is device-local only: session keys, migration flags, freshness watermarks, `FeatureFlags.rpgImmersive`, badge/icon eligibility. NEVER authoritative cross-device domain data (balances, counters, credits).
    ...
    - Concurrency Invariants:
      ...
      - The codebase strictly prohibits `@unchecked Sendable`. Concurrency boundaries are crossed using Sendable value types (e.g. `Date.FormatStyle`), `@MainActor` isolation, or native `actor` encapsulation (e.g. `BackgroundTaskFinisher`) rather than unsafe Sendable opt-outs.
...
│   │                               # + DEBUG-only SchemaSeedService seed door
...
**UI — decimal pad dismissal (cross-cutting write-time convention):** Every `TextField` using `.keyboardType(.decimalPad)` must provide a keyboard dismissal path. Pattern: `@FocusState private var isAmountFocused: Bool` on the view, `.focused($isAmountFocused)` on the `TextField`, and applying `View.decimalPadDoneToolbar(isFocused: $isAmountFocused, amountText: $amountText)` from `Project/Views/Shared/DecimalPadDismissModifier.swift`. The `amountText` parameter is optional: pass the bound amount string to also host the locale decimal-separator button (`decimalPad.separatorButton`), omit it for a Done-only bar. Implementation uses `.safeAreaInset(edge: .bottom)` anchored to `isFocused.wrappedValue` with `.scrollDismissesKeyboard(.interactively)` to host the interactive Done row plus separator button. UIKit's `ToolbarItemGroup(placement: .keyboard)` / `inputAccessoryView` is intentionally avoided because it generates non-finite frame dimension layout runtime faults (`Invalid frame dimension`) during sheet animations and keyboard transitions. This is a write-time convention — apply `decimalPadDoneToolbar(isFocused:amountText:)` when adding any new decimalPad field.

**UI — iCloud diagnostics visibility (prod-safe subset):** `iCloudStatusView` ships a prod-safe subset unconditionally in Release — cached record counts (all `*Cache` types scoped to the active family), `pendingUploadCount`, `lastSyncedAt` (relative), sync status, the "Last Change Received" row (`lastChangeReceivedAt`, engine-delivered change age), and the "Offline Changes Dropped" row plus the "Needs Refresh" status label/color/icon (`pendingBufferOverflowed` / `pendingBufferDroppedCount`) — so parents can self-triage sync health and see unresolved pending-buffer overflow loss without Debug builds. The two added surfaces are display-truth only: freshness watermarks stamp solely via `CKSyncEngineCoordinator.completeSyncPass()` after a full pass across all active scopes with zero failures, and neither the engine-delivered change marker nor the overflow status ever stamps freshness. Heavy diagnostics (engine state `private`/`shared`, per-scope freshness chips `✅/❌` per `CachedRecordType`, push/reconnect debounce ages, absolute timestamps, and `syncError` detail) remain gated behind `#if DEBUG` (`debugSyncHealthSection`). The `View` struct itself is unconditional, but the expensive debug overlay is compile-time excluded from Release. Do not re-wrap the entire view in `#if DEBUG`.
...
- Route EVERY engine-delivered and query-derived server→cache write through `ingest(...)` — sanctioned exceptions: the non-owner reconciliation door (`reconcileParticipantSet`, §4; the owner-side §4 pass rides ingest() via `handleIncomingRecordsDirectly`), FamilyService pre-session bootstrap mirrors (ingest fails closed before session establishment), and the DEBUG-only schema seed (`SchemaSeedService.pushSchemaSeed`, `#if DEBUG` only; reserved `schemaseed-` names in the active family zone only, never touches `CacheService`/`ingest`/`enqueue`, best-effort delete on all paths, `Family` never seeded).
...
- Don't write query results directly into CacheService/BackgroundCacheActor outside `ingest()` or the sanctioned doors: non-owner reconciliation (`reconcileParticipantSet`, §4), FamilyService pre-session bootstrap mirrors, and the DEBUG-only schema seed (`SchemaSeedService.pushSchemaSeed`, `#if DEBUG` only; reserved `schemaseed-` names, active-family zone only, never touches `CacheService`/`ingest`/`enqueue`, best-effort delete on all paths, `Family` never seeded).
