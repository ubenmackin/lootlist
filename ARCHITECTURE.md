...
- **Schema evolution** — the SwiftData cache schema is `LootListSchemaV6` (superseding `LootListSchemaV5`), on which `CacheService` constructs the `ModelContainer` using `LootListMigrationPlan`. SwiftData cache models use composite indexing `#Index([\.familyRecordName, \.recordName])` rather than global `@Attribute(.unique)` to isolate multi-tenant records across families while maintaining per-family uniqueness. Because SwiftData serves as an ephemeral read-through/write-through cache for CloudKit (the authoritative system of record), any incompatible on-disk SQLite constraint migration (such as dropping unique constraints between major schema versions) triggers an automatic, graceful cache store reset in `CacheService.init`, after which `CKSyncEngine` automatically rehydrates the cache from CloudKit.
...
### Data Migrations
- Schema/data backfills register a `MigrationStep` on `DataMigrationsCoordinator` (versioned via UserDefaults). Do not patch records ad-hoc inside services. (Note: the cache schema is `LootListSchemaV6` with `LootListMigrationPlan`; if an on-disk schema constraint conflict occurs during upgrade, `CacheService` executes an automatic destructive cache reset and clean container re-initialization, which `CKSyncEngine` immediately rehydrates from CloudKit — see §11.)
