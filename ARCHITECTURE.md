...
**In tests:** when `TestEnvironment.isRunningUnitOrUITests` is true, `CacheService(inMemory: true)` is used and `SampleData.populate(cacheService:)` seeds the in-memory SwiftData cache. There is no CloudKit mock layer inside the app — unit tests inject `MockCloudKitService` / `MockRecordStore` explicitly as protocol test-doubles (see §Testing), while the app process itself always runs the live `CloudKitService`.
...
- **Tests:** the production app has **no mock layer** — `CloudKitService` contains no `TestEnvironment` short-circuits, so the app process always runs the live service against the real container. Unit tests construct services against `MockCloudKitService` / `MockRecordStore`, injected explicitly as `any CloudKitServiceProtocol` test-doubles (`seedMockRecords` is a mock-only helper on `MockCloudKitService`, outside the protocol). UI tests launch the real app and therefore exercise the real `CloudKitService` against the container's Development environment; `LootListApp.seedTestData` populates only the local SwiftData cache plus deterministic auth state. Live CloudKit coverage also lives in the separate `LootListIntegrationTests` target (see §Testing).
...
Unit tests live in `ProjectTests/` (mirroring the source layout: `Models/`, `Services/`, `Utilities/`, `ViewModels/`); UI and snapshot tests live in `ProjectUITests/` (per-screen XCTestCases plus `LootListScreenshotTests` via `SnapshotHelper`); live CloudKit integration tests live in `ProjectIntegrationTests/` (the `LootListIntegrationTests` target). The exact test list is intentionally omitted — it is inspectable via tree/glob and grows with the codebase.

**Test-environment contract (this is the architectural part, not the file list):**
- **Test doubles are injected at the unit-test boundary, not short-circuited inside the app.** `CloudKitService` contains no `TestEnvironment` branches — the production app always runs the live service against the real container, and UI tests therefore exercise the real CloudKit path (Development environment). In unit tests, `MockCloudKitService` (backed by `MockRecordStore`) is constructed directly and passed into services as an explicit `any CloudKitServiceProtocol` test-double; its `seedMockRecords` helper pre-populates mock record state per-test and lives outside `CloudKitServiceProtocol`. What `TestEnvironment.isRunningUnitOrUITests` does gate **inside the app**: `CacheService` is constructed with `inMemory: true`; `CKSyncEngineCoordinator` skips instantiating real `CKSyncEngine`s; background reconcilers and sync/payout triggers are suppressed; and `LootListApp.seedTestData` seeds the **local SwiftData cache only** via `SampleData.populate(cacheService:)` plus a deterministic auth state — no CloudKit records are written by the seeding path.
- **Live CloudKit integration is a separate, deliberate target.** `LootListIntegrationTests` exercises a real `CloudKitService` against the live container (`iCloud.com.volcrypt.lootlist`) — entitlement resolution, zone lifecycle, record round-trips, and share flows that mocks cannot prove. It runs against the container's **Development environment only** (never Production) and is intentionally part of the developer workflow: each test creates a unique-per-run private zone (`TestZone_<uuid-prefix>`) via the `withTestZone` helper, which deletes the zone on both success and failure so no test data survives a run. Tests self-skip when no iCloud account is available (e.g., an unauthenticated simulator).
